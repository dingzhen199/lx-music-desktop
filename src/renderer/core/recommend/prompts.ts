/**
 * 听歌推荐引擎的提示词构建与 LLM 分析结果归一化纯逻辑。
 *
 * 语义移植自 from-here（MIT）bridge/prompts/music-semantic.js（AESTHETIC_CONSTITUTION/
 * ANALYSIS_SYSTEM/RANK_SYSTEM/buildAnchorAnalysisPrompt/buildRankingPrompt）与
 * bridge/server.js 的 normalizeAnalysis/recallQueries 纯函数。
 * 按 T-B0 规格做类型化重写与平台文案参数化：去掉“From Here/网易云”等字样，
 * 改为“本引擎/音乐平台”中性表达；语言类别（华语/日语/韩语）与曲风、听感词保留。
...
/** 一条召回方向（schema 兼容 snake_case 与 camelCase 输入，归一化后统一 camelCase）。 */
export interface RecallDirection {
  name: string
  reason: string
  aestheticBridge: string
  preserve: string[]
  drift: string[]
...
export interface TrackAnalysis {
  ...
  aesthetic: {
    why_it_stops_you: string
    human_state: string[]
    tension: string[]
    world: string
    unspoken: string
    avoid_reductions: string[]
    surprise_axes: string[]
  }
  fingerprint: {
    vocal_identity: string[]
    emotional_core: string[]
    imagery: string[]
    rhythm_motion: string[]
    dynamics: string[]
    instrumentation_texture: string[]
    melody_harmony: string[]
    narrative: string[]
    must_preserve: string[]
    can_drift: string[]
  }
  recallDirections: RecallDirection[]
  avoidTransforms: string[]
}

export const AESTHETIC_CONSTITUTION = `本引擎的审美宪法：
...
export function buildAnchorAnalysisPrompt({ anchor, radius, instruction = '' }: {
  anchor: AnchorLike
  radius: number
  instruction?: string
}): string {
...
/** 最近路径元素（排序输入）。 */
export interface RankPathInput {
  artist: string
  title: string
  journeyRole?: string
  distance?: number
...
export function buildRankingPrompt({ anchor, radius, instruction = '', analysis, candidates, recentPath = [] }: {
  ...
  const path = (recentPath || []).slice(-6).map(t => ({
    ...
    reason: t.reason ?? '',
    // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing -- 保留 from-here 的 || 回退：空串状态按 played 处理
...
/** 按 || 语义取第一个真值（from-here 中大量的 a||b 回退链）。 */
function truthy(...values: unknown[]): unknown {
...
function strArr(value: unknown, limit = 8): string[] {
  return asArray(value).slice(0, limit).map(String).filter(Boolean)
}

/**
 * 把 LLM 返回的分析归一化为 TrackAnalysis：
 * 兼容 snake_case/camelCase、v0.4.1 的 legacy searches/traits 与未知值回退。
 */
export function normalizeAnalysis(raw: unknown, anchor: AnchorLike): TrackAnalysis {
  const obj = asRecord(raw) ?? {}
  const fp = asRecord(obj.fingerprint) ?? {}
  const legacy = asRecord(obj.traits) ?? {}
  const dirsRaw = Array.isArray(obj.recall_directions) ? obj.recall_directions : (Array.isArray(obj.recallDirections) ? obj.recallDirections : [])
  const recallDirections: RecallDirection[] = dirsRaw.slice(0, 4).map((d, i) => {
    const dir = asRecord(d) ?? {}
    return {
      name: String(dir.name || `方向 ${i + 1}`),
      reason: String(dir.reason || ''),
      aestheticBridge: String(dir.aesthetic_bridge || dir.aestheticBridge || ''),
      preserve: strArr(dir.preserve, 6),
      drift: strArr(dir.drift, 6),
      searchArtists: strArr(dir.search_artists || dir.searchArtists, 6),
      searchKeywords: strArr(dir.search_keywords || dir.searchKeywords, 6),
      targetLanguage: String(dir.target_language || dir.targetLanguage || 'unknown').toLowerCase(),
    }
  }).filter(d => d.searchArtists.length || d.searchKeywords.length)
  // 兼容 v0.4.1 的简单 searches 格式。
  if (!recallDirections.length && Array.isArray(obj.searches)) {
    for (const [i, q] of obj.searches.slice(0, 4).entries()) {
      const keyword = typeof q === 'string' ? q : String(asRecord(q)?.keyword || '')
      if (keyword) {
        recallDirections.push({
          name: `方向 ${i + 1}`,
          reason: String(asRecord(q)?.reason || ''),
          aestheticBridge: '',
          preserve: [],
          drift: [],
  ...
  const fingerprint = {
    vocal_identity: strArr(truthy(fp.vocal_identity, legacy.vocal ? [legacy.vocal] : []), 8),
    emotional_core: strArr(truthy(fp.emotional_core, legacy.mood), 8),
    imagery: strArr(fp.imagery, 8),
    rhythm_motion: strArr(fp.rhythm_motion, 8),
    dynamics: strArr(fp.dynamics, 8),
    instrumentation_texture: strArr(truthy(fp.instrumentation_texture, legacy.texture), 8),
    melody_harmony: strArr(fp.melody_harmony, 8),
    narrative: strArr(fp.narrative, 8),
    must_preserve: strArr(truthy(fp.must_preserve, legacy.invariants), 6),
    can_drift: strArr(fp.can_drift, 8),
  }
  const langRaw = asRecord(truthy(obj.anchor_language, obj.anchorLanguage)) ?? {}
  const anchorLanguage = {
    code: String(truthy(langRaw.code, langRaw.language) || 'unknown').toLowerCase(),
    confidence: String(truthy(langRaw.confidence) || 'low').toLowerCase(),
    reason: String(truthy(langRaw.reason) || ''),
  }
  const aesRaw = asRecord(obj.aesthetic) ?? {}
  const aesthetic = {
    why_it_stops_you: String(truthy(aesRaw.why_it_stops_you, aesRaw.whyItStopsYou) || ''),
    human_state: strArr(truthy(aesRaw.human_state, aesRaw.humanState), 6),
    tension: strArr(aesRaw.tension, 6),
    world: String(truthy(aesRaw.world) || ''),
    unspoken: String(truthy(aesRaw.unspoken) || ''),
    avoid_reductions: strArr(truthy(aesRaw.avoid_reductions, aesRaw.avoidReductions), 6),
    surprise_axes: strArr(truthy(aesRaw.surprise_axes, aesRaw.surpriseAxes), 6),
  }
  const avoidRaw = strArr(truthy(obj.avoid_transforms, obj.avoidTransforms), 8)
  return {
    summary: String(truthy(obj.summary) || `${anchor.artist} — ${anchor.title}`),
    anchorLanguage,
    aesthetic,
    fingerprint,
    recallDirections,
    avoidTransforms: avoidRaw.length ? avoidRaw : ['tribute', 'karaoke', 'instrumental reinterpretation'],
  }
}

/** 从分析结果生成召回查询（去重、按半径限量、reason 拼接）。 */
export function recallQueries(
  ...
  const out: Array<{ keyword: string, reason: string }> = []
  ...
  for (const d of analysis?.recallDirections ?? []) {
    ...
    for (const term of terms) {
      ...
      seen.add(key)
      out.push({ keyword, reason: [d.aestheticBridge, d.reason, d.name].filter(Boolean).join('；') })
