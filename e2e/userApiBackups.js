...
async function startAudioServer() {
  ...
  const file = fs.readdirSync(mediaDir).find(f => f.endsWith('.wav'))
  if (!file) throw new Error('dist/media 无 wav 文件')
...
;(async() => {
  ...
  await run(window, 'U3-主源失败→换提供方→备源接管播放（网络敏感）', async() => {
    ...
    if (!rowCount) throw new Error('搜索结果 0 行（网络受限）')

    // 右键第一行 → 添加到... → 我的收藏（避免先播放导致 URL 缓存短路回退链）
    await window.locator('.list-item').first().click({ button: 'right', position: { x: 250, y: 10 } })
    await window.waitForTimeout(900)
    ...
    try {
      ...
      record('U3a-状态栏出现切换提示（尽力捕获）', statusSeen.size > 0, [...statusSeen].join(' | ') || '未捕获（轮询间隔错过瞬时文案）')

      // U4 写回：love 列表中该歌曲 source 不再是 kw（换到 findMusic 候选提供方）
      let sourceAfter = ''
      for (let i = 0; i < 10; i++) {
        await window.waitForTimeout(1000)
        sourceAfter = sqlite(profileDir, "SELECT source FROM my_list_music_info WHERE listId='love' LIMIT 1")
        if (sourceAfter != 'kw') break
      }
  ...
  // ================= U5 全程错误 =================
  await run(window, 'U5-全程无未预期错误', async() => {
  ...
  await screenshot(window, 'userApiBackups_final')
  console.log('\n===== 全部错误记录（含可容忍） =====')
